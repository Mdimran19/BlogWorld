  ...
  {blogs.map((blog) => (
    <tr key={blog._id} className='bg-white border-b'>
      ...
      <td className='flex gap-x-2'>
        <button  onClick={() => handleDelete(blog._id)} className='border border-black rounded-sm  active:bg-gray-600 active:text-white cursor-pointer'>delete</button>
        <Link to={`/admin/edit/${blog._id}`} >
        <button className='border border-black rounded-sm  active:bg-gray-600 active:text-white cursor-pointer'>edit</button>
        </Link>
      </td>
    </tr>
  ))}
