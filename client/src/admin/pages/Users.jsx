        ...
        <table className='w-full text-sm text-gray-500 p-5'>
          <thead className='text-sm text-gray-700 text-left uppercase bg-gray-50'>
            <tr className='flex justify-between'>
              <th scope='col' className=' sm:block px-6 py-3'>User Name</th>
              <th scope='col' className=' sm:block px-6 py-3'>User Email</th>
          ...
          <tbody>
            {usersArray.map((user) => (
              <tr key={user._id} className='bg-white border-b flex justify-between'>
                <td  className='items-center gap-3 md:flex sm:flex px-6 py-2 font-medium text-gray-900 whitespace-nowrap'>
                {user.name ? user.name : "No title"}
               
                </td>
                 <td className='items-center gap-3  sm:flex px-6 py-2 font-medium text-gray-900 whitespace-nowrap'> {user.email ? user.email : "No title"}</td>
                <td className='flex gap-x-2 pr-3'>
                  <button onClick={() => handleDelete(user._id)} className='border px-3  border-black rounded-sm active:bg-gray-600 active:text-white cursor-pointer'>
                    delete
                  </button>
                </td>
