        ...
        <table className='w-full text-sm text-gray-500 p-5'>
          <thead className='text-sm text-gray-700 text-left uppercase bg-gray-50'>
            <tr className='flex justify-between'>
              <th scope='col' className=' sm:block px-6 py-3'>Email Name</th>
              <th scope='col' className='px-6 py-3 pr-6'>Action</th>
            </tr>
          </thead>
          <tbody>
  {emails.map((email) => (
    <tr key={email._id} className='bg-white border-b flex flex-nowrap justify-between'>
      <td scope='row' className='items-center gap-3  sm:flex px-6 py-2 font-medium text-gray-900 whitespace-nowrap'>
      {email.email}
      </td>
     
      <td className='flex gap-x-2 pr-3'>
        <button  onClick={() => handleDelete(email._id)} className='border px-3 border-black rounded-sm  active:bg-gray-600 active:text-white cursor-pointer'>delete</button>
